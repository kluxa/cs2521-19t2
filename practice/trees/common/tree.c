...
static int strToIntArray(char *s, int *arr);
...
Tree readTree(int *treeNo) {
	...
	int arr1[1024];
	int size1 = readIntArray(arr1, 1024);
	
	readTreePrompt("in-order", treeNo);
	int arr2[1024];
	int size2 = readIntArray(arr2, 1024);
...
static int readIntArray(int *arr, int maxInts) {
	char buf[maxInts * 20];
	fgets(buf, maxInts * 20, stdin);
	return strToIntArray(buf, arr);
}
...
static bool isBst(Tree t, int min, int max);

void ensureTreeIsBst(Tree t) {
	if (!isBst(t, INT_MIN, INT_MAX)) {
		fprintf(stderr, "error: tree is not a bst\n");
		exit(EXIT_FAILURE);
	}
}
...
static bool isBst(Tree t, int min, int max) {
	...
	return (isBst(t->left, min, t->value - 1) &&
	        isBst(t->right, t->value + 1, max));
}
